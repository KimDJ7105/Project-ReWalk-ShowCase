...
void GraphicsDescriptorHeap::Init(uint32 count)
{
	...
	desc.NumDescriptors = count * (CBV_SRV_REGISTER_COUNT - 1);		// b0는 전역이기 때문에 레지스터 갯수 -1개를 한다
	...
	_groupSize = _handleSize * (CBV_SRV_REGISTER_COUNT - 1);		// b0는 전역이기 때문에 레지스터 갯수 -1개를 한다
}
